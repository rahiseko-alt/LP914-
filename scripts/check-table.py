...
def official_l1():
    """同梱した公式データを読み、項目 ID -> 章 ID の対応を返す。"""
    ...
    return {"v5.0.0-" + row["req_id"].lstrip("V"): row["chapter_id"] for row in rows}
...
def main():
    ...
    for path in tables:
        ...
        for chapter in sorted({official[i] for i in present if i in official}):
            missing = {i for i, c in official.items() if c == chapter} - present
